...
function hasWon(board, player) {
    board.forEach((row) => {
        if (row.filter((x) => x === player).length === 3)
            return true;
    });
    for (let i = 0; i < 3; ++i) {
        if (board[0][i] === player &&
...
function doMove(board, move, player) {
    const row = move[0];
    const col = move[1];
...
function minimax(inBoard, isMaximizing, depth) {
    if (depth === 0 || isGameOver(inBoard))
        return [evaluate(inBoard), []];
    let bestMove = [];
    let bestValue;
    let symbol;
    if (isMaximizing) {
